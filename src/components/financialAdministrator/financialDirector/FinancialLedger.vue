      ...
      <div>
        <el-form style="float: left" :model="totalForm" ref="totalForm" :inline="true">
          <el-form-item label="收入合计:">
            <el-input v-model="totalForm.incomeTotal"></el-input>
          </el-form-item>
          <el-form-item label="支出合计:">
            <el-input v-model="totalForm.expenseTotal"></el-input>
          </el-form-item>
          <el-form-item label="结余合计:">
            <el-input v-model="totalForm.profitTotal"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <div style="float: right">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        gridOptions: {
          ...
          columnDefs: [
            ...
            {
              headerName: '收入', width: 150, field: 'income', filter: 'text', hide: false, visible: true
            },
            {
              headerName: '支出', width: 150, field: 'expense', filter: 'text', hide: false, visible: true
            },
            {
              headerName: '结余', width: 150, field: 'profit', filter: 'text', hide: false, visible: true
            },
            {
    ...
    methods: {
      ...
      createRowData (i) {
        if (i === 1) {
          this.gridOptions.rowData = testJson.financialLedger.list
          this.gridOptions.api.setRowData(this.gridOptions.rowData)
        } else if (i === 2) {
          this.gridOptions2.rowData = testJson.financialLedger.list
    ...
    computed: {
      ...
      calculateMoney () {
        if (this.verVisible === false) {
          this.totalForm.incomeTotal = 0
          this.totalForm.expenseTotal = 0
          this.totalForm.profitTotal = 0
          let model = this.gridOptions.api.getModel()
          let arr = model.rootNode.childrenAfterFilter
          console.log(arr[0].data)
          for (let i = 0; i < arr.length; i++) {
            this.totalForm.incomeTotal += arr[i].data.income
            this.totalForm.expenseTotal += arr[i].data.expense
            this.totalForm.profitTotal += arr[i].data.profit
          }
        }
      }
    },
