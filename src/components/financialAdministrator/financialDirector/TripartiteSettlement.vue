    ...
    <div>
      ...
      <div>
        <el-form style="float: left" :model="totalForm" ref="totalForm" :inline="true">
          <el-form-item label="三方收入合计:">
            <el-input v-model="totalForm.trilIncomeTotal"></el-input>
          </el-form-item>
          <el-form-item label="三方支出合计:">
            <el-input v-model="totalForm.trilExpenseTotal"></el-input>
          </el-form-item>
          <el-form-item label="三方盈收合计:">
            <el-input v-model="totalForm.trilProfitTotal"></el-input>
          </el-form-item>
        </el-form>
      </div>
      <div style="float: right">
...
<script>
  ...
  export default {
      ...
      return {
        ...
        gridOptions: {
          ...
          columnDefs: [
            ...
            {
              headerName: '三方收入', width: 150, field: 'trilIncome', filter: 'text', hide: false, visible: true
            },
            {
              headerName: '三方支出', width: 150, field: 'trilExpense', filter: 'text', hide: false, visible: true
            },
            {
              headerName: '三方盈收', width: 150, field: 'trilProfit', filter: 'text', hide: false, visible: true
            },
            {
              headerName: '审核人', width: 150, field: 'auditor', filter: 'text', hide: false, visible: true
            },
            {
              headerName: '审核日期', width: 150, field: 'auditTim', filter: 'text', hide: false, visible: true
            }
          ]
        },
        gridOptions2: {
        ...
        // 各种费用合计
        totalForm: {
          trilIncomeTotal: 0, // 三方收入合计
          trilExpenseTotal: 0, // 三方支出合计
          trilProfitTotal: 0 // 三方盈收合计
        },
        // 核销完成后提交给后台的数据
        confirmSubForm: {
          orderId: []
    ...
    methods: {
      ...
      createRowData (i) {
        if (i === 1) {
          this.gridOptions.rowData = testJson.tripartiteSettlement.list
          this.gridOptions.api.setRowData(this.gridOptions.rowData)
        } else if (i === 2) {
          this.gridOptions2.rowData = testJson.tripartiteSettlement.list
    ...
    computed: {
      ...
      calculateMoney () {
        if (this.verVisible === false) {
          this.totalForm.trilIncomeTotal = 0
          this.totalForm.trilExpenseTotal = 0
          this.totalForm.trilProfitTotal = 0
          let model = this.gridOptions.api.getModel()
          let arr = model.rootNode.childrenAfterFilter
          console.log(arr[0].data)
          for (let i = 0; i < arr.length; i++) {
            this.totalForm.trilIncomeTotal += arr[i].data.trilIncome
            this.totalForm.trilExpenseTotal += arr[i].data.trilExpense
            this.totalForm.trilProfitTotal += arr[i].data.trilProfit
          }
        }
      }
    },
