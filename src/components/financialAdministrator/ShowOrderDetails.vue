  ...
  <div>
    <el-form ref="form" :model="form">
      <div style='margin-top:0.5%;display:inline-block;width:100%'>
        ...
        <span style='float:left;padding-top:0.5%' class='col-1'>{{this.orderId}}</span>
        <span class='float:left;col-1 order-title-base'>经办客服：</span>
      ...
      <span class='col-1 label-content' style='color:#00d1b2'>{{this.orderId}}</span>
      ...
      <span class='label-content' v-for="(value, key, i) in form" :key='key' v-if="i>72&&i<78"
            :class="i==76?'col-2':'col-1'">{{key}}</span>
      <span
        class='col-1 label-content'>{{form.feeMoney[0] + form.feeMoney[1] + form.feeMoney[2] + form.feeMoney[3]}}</span>
      ...
      <span class='col-1 label-title'
            v-bind:style="{height:(form.transfor.length+1)*38+'px',paddingTop: form.transfor.length*2.25 + '%' }">长途运输</span>
